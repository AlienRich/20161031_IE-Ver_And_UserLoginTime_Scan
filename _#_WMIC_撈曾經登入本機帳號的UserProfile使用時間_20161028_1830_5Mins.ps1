...
$QueryPC = Get-WmiObject -Class Win32_UserProfile -ComputerName $IE8IP -errorvariable errorvar|select PSComputerName,@{N='UserID';E={(($_.LocalPath).replace("D:\Users\","")).replace("C:\Users\","")}},@{N='LastUseTime';E={($_.LastUseTime).Substring(0,12)}},SID | where SID -like "S-1-5-21-*" | sort LastUseTime -Descending
if (-not $errorvar)
{
	$PCUserIP = $QueryPC
	...
	write-host "$RunDayTime ::: $IE8IP ::: Query-OK!!" -ForegroundColor GREEN
	return
	$IE8IP="NOMSG"
	$RunDayTime="NOMSG"
	$QueryPC="NOMSG"
	...
	$1UserIDandPCName="NOMSG"
		...
		write-host "$RunDayTime ::: $IE8IP ::: TimeOut!!" -ForegroundColor Red
		[Environment]::Exit(1)
}
