...
###		∏—ª°°Ghttp://ramblingcookiemonster.github.io/Join-Object/

###		Ωd®“-----------•™TableA--------•kTableB---Table1A§ÒπÔKEY"UserID"---Table1B§ÒπÔKEY"UserID"----•™Table1A≈„•‹ƒÊ¶Ï~~~~~~~~~~~~~~~~~~~~•kTable1B≈„•‹ƒÊ¶Ï------------
###		Join-Object -Left $DATA -Right $PCUserIP -LeftJoinProperty UserID -RightJoinProperty UserID -LeftProperties PSComputerName,UserID -RightProperties LastUseTime -Type AllInBoth -Prefix r_

###		∞—º∆ª°©˙...
###		AllInLeft°G		(Left joins)•™√‰•˛≥°≈„•‹°A•k√‰®S¶≥∑|¶®¨∞™≈≠»
###		OnlyIfInBoth°G(Inner joins)•™•k®‚√‰≥£≤≈¶X§~≈„•‹°A®S¶≥¶@¶P≠»™∫§£∑|º¥•X®”
###		AllInBoth°G		(Full joins)®‚√‰≥£≈„•‹°A®‚√‰®S¶≥™∫∑|•H™≈≠»™Ì•‹°C

###		-Prefix r_°G•k√‰™∫ƒÊ¶Ï•[§Wµ˘∏—"r_"
###		PSComputerName             UserID                     r_LastUseTime
###		--------------             ------                     -------------
###		P104PC02HKXT               ithelp                     201610190050
###		P104PC02HKXT               089967                     201607210348
###		P104PC02HKXT               cdib2485                   201607210348
###		P104PC02HKXT               CSMBAdmin.P104PC02HKXT     201610250058
...
write-host "$RunDayTime ::: $IE8IP" -ForegroundColor GREEN

###	Privileges°G±b∏πƒ›© ßt∑N--->Guest=(0)?ÅUser=(1)?ÅAdministrator=(2)
###	$QueryPC = Get-WmiObject -Class win32_networkloginprofile -Computer $IE8IP | Select-Object -Unique -Property PSComputerName,Name,Caption,Privileges,FullName,@{N='LastLogonTime';E={($_.LastLogon).replace(".000000+480","")}},@{N='PasswordExpireDate';E={($_.PasswordExpires).replace(".000000+480","")}} | where {$_.Privileges -cnotlike ""} | sort LastLogon,PasswordExpires -Descending
###	$QueryPC = Get-WmiObject -Class win32_networkloginprofile -Computer $IE8IP | Select-Object -Unique -Property PSComputerName,Name,Caption,Privileges,FullName,@{N='LastLogonTime';E={($_.LastLogon).replace(".000000+480","")}},@{N='PasswordExpireDate';E={($_.PasswordExpires).replace(".000000+480","")}} | sort LastLogon,PasswordExpires -Descending
###	$PCinfo = $QueryPC | where {$_.Name -like "KGIBANK\0*" -or $_.Name -like "KGIBANK\1*" -or $_.Name -like "KGIBANK\2*"} | select * -Unique
###	$IE8IP = "172.17.8.190"

$QueryPC = Get-WmiObject -Class Win32_UserProfile -ComputerName $IE8IP |`
###	select PSComputerName,@{N='UserID';E={(($_.LocalPath).replace("D:\Users\","")).replace("C:\Users\","")}},`
###	select PSComputerName,@{N='UserID';E={((($_.LocalPath).replace("D:\Users\","")).replace("C:\Users\","")).replace(".KGIBANK","")}},`
	select PSComputerName,@{N='UserID';E={($_.LocalPath).Split(":\.")[3]}},`
	@{N='LastUseTime';E={($_.LastUseTime).Substring(0,12)}},SID |`
	where SID -like "S-1-5-21-*" | sort LastUseTime -Descending

$PCUserIP = $QueryPC
$PCUserIP | Add-Member -MemberType NoteProperty -Name ScanIP -Value "$IE8IP" -Force
$PCUserIP | Export-Csv -NoTypeInformation -Encoding UTF8 -Append -PATH .\_$DayTime-PC-UserProfile_UseTime.csv

$ShowUser="NOMSG"
	$PCUserIP | Foreach-Object { $ShowUser = $_
	write-host "$ShowUser" -ForegroundColor CYAN}

$AllUserID = $PCUserIP | select UserID
...
$data = @()
$PCUserIP.UserID | Foreach-Object { $1UserID = $_
...
#####################################################
$1UserIDandPCName = $PCUserIP
$1UserIDandPCName | Add-Member -MemberType NoteProperty -Name UserDomainInfo $UserAllInfo -Force
$row = "" | Select PSComputerName,UserID,UserDomainID,UserName,ScanIP
$row.PSComputerName=$PCUserIP.PSComputerName[0]
$row.UserID=$1UserID
$row.UserDomainID=$UserDomainID
...
$PCAndUserInfoJoin = Join-Object -Left $DATA -Right $1UserIDandPCName `
	-LeftJoinProperty UserID -RightJoinProperty UserID `
	-LeftProperties PSComputerName,UserID,UserDomainID,UserName,ScanIP `
	-RightProperties LastUseTime,SID -Type AllInBoth -Prefix inR_ `
	| where {$_.UserID -like "1*" -or $_.UserID -like "0*" -or $_.UserID -like "2*"} `
	| sort -Descending inR_LastUseTime

$PCAndUserInfoJoin | Export-Csv -NoTypeInformation -Encoding UTF8 -Append -PATH .\_$DayTime-PC-UserProfile_UseTime_PCAndUserInfoJoin.csv
