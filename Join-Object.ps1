...
###		解說：http://ramblingcookiemonster.github.io/Join-Object/

###		範例-----------左TableA--------右TableB---Table1A比對KEY"UserID"---Table1B比對KEY"UserID"----左Table1A顯示欄位~~~~~~~~~~~~~~~~~~~~右Table1B顯示欄位------------
...
###		參數說明...
###		AllInLeft：		(Left joins)左邊全部顯示，右邊沒有會成為空值
###		OnlyIfInBoth：(Inner joins)左右兩邊都符合才顯示，沒有共同值的不會撈出來
###		AllInBoth：		(Full joins)兩邊都顯示，兩邊沒有的會以空值表示。

###		-Prefix r_：右邊的欄位加上註解"r_"
